...
# ─── Subscription Tiers ───
SUBSCRIPTION_TIERS = {
    "free": {"name": "Free", "hustle_limit": 2, "price": 0.00},
    "starter": {"name": "Starter", "hustle_limit": 10, "price": 9.99},
    "pro": {"name": "Pro", "hustle_limit": 999999, "price": 29.99},
}
...
async def generate_hustles(user: dict = Depends(get_current_user)):
    tier = SUBSCRIPTION_TIERS.get(user.get("subscription_tier", "free"), SUBSCRIPTION_TIERS["free"])
    current_count = await db.side_hustles.count_documents({"user_id": user["user_id"]})
    if current_count >= tier["hustle_limit"]:
        raise HTTPException(
            status_code=403,
            detail=f"You've reached your {tier['name']} plan limit of {tier['hustle_limit']} side hustles. Upgrade to unlock more!"
        )
    ...
    remaining = tier["hustle_limit"] - current_count
    num_to_generate = min(5, remaining)
...
Return ONLY valid JSON array, no other text."""
    ...
    await db.users.update_one(
        {"user_id": user["user_id"]},
        {"$set": {"hustle_count": current_count + len(created_hustles)}}
    )
...
async def get_profile(user: dict = Depends(get_current_user)):
    tier_info = SUBSCRIPTION_TIERS.get(user.get("subscription_tier", "free"), SUBSCRIPTION_TIERS["free"])
    hustle_count = await db.side_hustles.count_documents({"user_id": user["user_id"]})
    selected_count = await db.side_hustles.count_documents({"user_id": user["user_id"], "selected": True})
    return {
        "user": user,
        "subscription": {
            "tier": user.get("subscription_tier", "free"),
            "name": tier_info["name"],
            "hustle_limit": tier_info["hustle_limit"],
            "price": tier_info["price"],
        },
        "stats": {
            "total_hustles": hustle_count,
            "selected_hustles": selected_count,
            "remaining": max(0, tier_info["hustle_limit"] - hustle_count),
        }
    }
